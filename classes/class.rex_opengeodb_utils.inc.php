...
class rex_opengeodb_utils {
	public static function getSettingsFile() {
		...
		$dataDir = $REX['INCLUDE_PATH'] . '/data/addons/opengeodb/';
	...
	public static function updateSettingsFile($showSuccessMsg = true) {
		...
		if ($msg != '') {
			...
			foreach ((array) $REX['ADDON']['opengeodb']['settings'] as $key => $value) {
				$content .= "\$REX['ADDON']['opengeodb']['settings']['$key'] = " . var_export($value, true) . ";\n";
			}

			if (rex_put_file_contents($settingsFile, $content)) {
				if ($REX['REDAXO'] && $showSuccessMsg) {
					echo rex_info($I18N->msg('opengeodb_config_ok'));
				}
			} else {
				if ($REX['REDAXO']) {
					echo rex_warning($I18N->msg('opengeodb_config_error'));
				}
			}
		}
	}
	...
	public static function replaceSettings($settings) {
		...
		// type conversion
		foreach ($REX['ADDON']['opengeodb']['settings'] as $key => $value) {
			if (isset($settings[$key])) {
				$settings[$key] = self::convertVarType($value, $settings[$key]);
			}
		}

		$REX['ADDON']['opengeodb']['settings'] = array_merge((array) $REX['ADDON']['opengeodb']['settings'], $settings);
	}
	...
	public static function checkDir($dir) {
		...
		if (!@is_dir($path)) {
			if ($REX['REDAXO']) {
				return $I18N->msg('opengeodb_install_make_dir', $dir);
			}
		} elseif (!@is_writable($path . '/.')) {
			if ($REX['REDAXO']) {
				return $I18N->msg('opengeodb_install_perm_dir', $dir);
			}
		}
